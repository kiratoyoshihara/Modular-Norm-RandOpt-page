...
export const ITERATIVE_LAYOUT = { width: 540, height: 420, left: 80, top: 76, plotWidth: 430, plotHeight: 286, maxN: 3200 };
export const iterativeX = (n) => n / ITERATIVE_LAYOUT.maxN * ITERATIVE_LAYOUT.plotWidth;
...
export function iterativeComparison(task) {
  ...
  return ours && baseline ? { ours, baseline, ratio: baseline.n / ours.n, difference: ours.mean - baseline.mean } : null;
}
...
export function renderIterativePanel(data, task, { interactive = true } = {}) {
  ...
  const series = [...data.methods].reverse().map((method) => {
    const points = task.points.filter((p) => p.method === method.id && p.mean != null).sort((a,b) => a.n-b.n);
    if (!points.length) return '';
    const dots = points.map((p) => {
      const x = num(iterativeX(p.n)), y = num(iterativeY(p.mean,task.domain));
      const label = `${method.label}, ${task.label}: ${p.mean.toFixed(2)} ± ${p.sd.toFixed(2)} percent; N=${p.n}, K=${p.k}.`;
      const attrs = interactive ? `role="button" tabindex="0" aria-label="${esc(label)}" aria-controls="iterative-tooltip" data-iterative-point="${task.id}-${p.id}" data-method="${method.id}" data-label="${esc(method.label)}" data-task="${task.label}" data-mean="${p.mean}" data-sd="${p.sd}" data-n="${p.n}" data-k="${p.k}" style="--method-color:${method.color}"` : '';
  ...
  const ticks = [0,1000,2000,3000].map((tick) => `<path d="M${num(iterativeX(tick))} ${l.plotHeight}v5" stroke="#d0d3d8"/><text x="${num(iterativeX(tick))}" y="${l.plotHeight+26}" text-anchor="middle">${tick.toLocaleString('en-US')}</text>`).join('');
  const comparison = iterativeComparison(task);
  const emphasis = comparison ? `<g class="f2-emphasis" pointer-events="none"><text class="iterative-callout" x="0" y="-50">${comparison.ours.mean.toFixed(2)}% at N=100</text><text class="iterative-comparison" x="0" y="-28">${num(comparison.ratio)}× smaller N vs ES-at-Scale · ${comparison.difference >= 0 ? '+' : '−'}${Math.abs(comparison.difference).toFixed(2)} pp</text>${task.points.filter((p) => p.method === 'modular' && p.n <= 100 && p.mean != null).map((p) => `<text class="iterative-small-n" x="${num(iterativeX(p.n) + 17)}" y="${num(iterativeY(p.mean,task.domain) + (p.n === 25 ? 22 : -12))}">N=${p.n}</text>`).join('')}<circle cx="${num(iterativeX(comparison.ours.n))}" cy="${num(iterativeY(comparison.ours.mean,task.domain))}" r="11" fill="none" stroke="#306FAD" stroke-width="1"/></g>` : '';
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${l.width} ${l.height}" class="iterative-svg" role="${interactive ? 'group' : 'img'}" aria-label="${task.label}: accuracy versus N" aria-describedby="${id}-desc">
    <desc id="${id}-desc">${task.testExamples} test examples. Points show mean accuracy over three seeds. The horizontal axis is linear in N: sampled candidates for RandOpt methods, perturbations for iterative baselines. MN-RandOpt is shown at N=100, K=25; N=3000, K=25; and N=3000, K=1. Iterative methods and the MN K=1 control return single models. Missing scores are omitted, not plotted as zero.</desc>
